...
export class Container {
     ...
     * @param template
     */
    constructor(width, height, rel_x = 0, rel_y = 0, template = null) {
        this.element = null;
        this.parent = null;
        this.template = null;
        this.width = width;
...
export class Connection extends Container {
    static UndetailedConstructor(from, to, template, style = "solid", ending = "arrow") {
        let fromCenter = { x: Math.floor(from.rel_x + from.width / 2), y: Math.floor(from.rel_y + from.height / 2) };
        let toCenter = { x: Math.floor(to.rel_x + to.width / 2), y: Math.floor(to.rel_y + to.height / 2) };
    ...
    update() {
        ...
        let fromCenter = { x: Math.floor(this.from.rel_x + this.from.width / 2), y: Math.floor(this.from.rel_y + this.from.height / 2) };
        let toCenter = { x: Math.floor(this.to.rel_x + this.to.width / 2), y: Math.floor(this.to.rel_y + this.to.height / 2) };
        let width = Math.abs(toCenter.x - fromCenter.x);
        let height = Math.abs(toCenter.y - fromCenter.y);
        let length = Math.sqrt(width * width + height * height);
        if (length === 0) {
    ...
    constructor(width, height, dirVector, rel_x, rel_y, template) {
