...
export default class PrefabAnimations {
    ...
    static BUBBLE_SORT_ANIMATION(vector, compFunction, renderer, speed) {
    ...
    static INSERTION_SORT_ANIMATION(vector, compFunction, renderer, speed) {
        let array = PrefabAnimations.init(vector, speed);
        let n = array.length;
        let colors = array.map(item => item.container.template.style.backgroundColor);
        for (let i = 0; i < n - 1; i++) {
    ...
    static QUICK_SORT_ANIMATION(vector, compFunction, renderer, speed) {
        let array = PrefabAnimations.init(vector, speed);
        let n = array.length;
        let colors = array.map(item => item.container.template.style.backgroundColor);
