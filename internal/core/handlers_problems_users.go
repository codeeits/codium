...
func (cfg *ApiCfg) UpdateSolutionTestsHandler(w http.ResponseWriter, r *http.Request, solution database.Solution, sendingUser database.User) {
	...
	if int32(len(p.GivenAnswers)) != problem.TotalTests {
		cfg.Logger.Printf("Invalid number of given answers for problem: %v", problem.TotalTests)
		http.Error(w, "Invalid number of given answers", http.StatusBadRequest)
		return
	}
	...
	if problem.TotalTests == testsPassed {
