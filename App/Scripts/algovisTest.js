...
// Make sure window.AlgoVis is available just in case
window.AlgoVis = window.AlgoVis || AlgoVis;

const { Viewport, RootContainer, Container, AnimationHandler, COMMON_ANIMATIONS, COMMON_ANIMATION_EASING_FUNCTIONS } = AlgoVis;

document.addEventListener("DOMContentLoaded", () => {
    let view = new Viewport(1220, 800, 20, 0, "#1B0524", "black", 2);
    ...
    for (let i = 0; i < n; i++) {
        ...
        element.style.lineHeight = size_y + "px"; // vertically center text
    ...
    function stopDrag(e) {
        if (!dragging) return;
        try { viewPortElement.releasePointerCapture(activePointerID); } catch (err) {}
        dragging = false;
    ...
    let vector_copies = vector.map(v => ({...v}));
    for (let i = 0; i < n  - 1; i++) {
        for (let j = i + 1; j < n; j++) {
    ...
    // Start the animation loop after a short delay so the user can see it
    setTimeout(() => {
        animator.Start();
    }, 1000);
});